...
export class AppComponent {
  ...
  llamadoEjemplo() {
    ...
    this.sexShopservice.llamadoHttp(tipo, sql).subscribe(
      //el subscribe hace que se espere respuesta
      (data: any) => {
        //data es todo lo que devuelve como respuesta el llamado al webservice
        console.log(data); // se usan los datos ( en este caso se muestra la res. en la consola del navegador)

        if (data.success == true) {
          //valida si el webservide me responde de manera correcta
      ...
      (error: any) => {
        //si hubo algun error en el llamado se guarda aqui en error
